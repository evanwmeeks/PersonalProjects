import os
import sys
import boto3
import tkinter as tk
from tkinter import filedialog, messagebox, simpledialog

# Get the directory of the Python script
script_dir = os.path.dirname(os.path.abspath(__file__))


def browse_files():
    """Opens a new finder window and allows the user to select files for upload. Closes program if Cancel button
    selected"""
    root = tk.Tk()
    root.withdraw()
    file_paths = filedialog.askopenfilenames()
    root.destroy()
    if not file_paths:
        messagebox.showinfo("Information", "File selection canceled. Script terminated.")
        sys.exit()
    return file_paths


def check_aws_credentials(access_key, secret_key):
    """Verifies existing Wasabi credentials by attempting to list the names of buckets
    inside the root Wasabi folder. Returns True or False depending on login result"""
    try:
        session = boto3.Session(
            aws_access_key_id=access_key,
            aws_secret_access_key=secret_key
        )
        s3 = session.resource(
            's3',
            endpoint_url= "OBFUSCATED FOR DEMO PURPOSES"
        )
        s3.meta.client.list_buckets()  # Verify credentials by listing buckets
        return True
    except Exception:
        return False


def prompt_aws_credentials():
    """Prompts user for Wasabi Access and Secret Keys"""
    root = tk.Tk()
    root.withdraw()
    access_key = simpledialog.askstring("Wasabi Credentials", "Enter your Wasabi access key:")
    if access_key is None:
        messagebox.showinfo("Information", "Wasabi credentials required. Script terminated.")
        sys.exit()
    secret_key = simpledialog.askstring("Wasabi Credentials", "Enter your Wasabi secret key:")
    if secret_key is None:
        messagebox.showinfo("Information", "Wasabi credentials required. Script terminated.")
        sys.exit()
    return access_key, secret_key


def create_aws_credentials_file(access_key, secret_key):
    """Creates new AWS Credential file inside the user's computer's root directory (~/.aws/credentials)"""
    aws_credentials_path = os.path.expanduser("~/.aws/credentials")
    os.makedirs(os.path.dirname(aws_credentials_path), exist_ok=True)
    with open(aws_credentials_path, "w") as f:
        f.write("[default]\n")
        f.write(f"aws_access_key_id = {access_key}\n")
        f.write(f"aws_secret_access_key = {secret_key}\n")


def read_aws_credentials_file():
    """Reads  ~/.aws/credentials file, sets the Access and Secret keys into vars and returns each"""
    aws_credentials_path = os.path.expanduser("~/.aws/credentials")
    if not os.path.exists(aws_credentials_path):
        return None, None

    with open(aws_credentials_path, "r") as f:
        lines = f.readlines()

    access_key = None
    secret_key = None
    for line in lines:
        if line.strip().startswith("aws_access_key_id"):
            access_key = line.strip().split("=")[1].strip()
        elif line.strip().startswith("aws_secret_access_key"):
            secret_key = line.strip().split("=")[1].strip()

    return access_key, secret_key


# Begin program
root = tk.Tk()
root.withdraw()

access_key, secret_key = read_aws_credentials_file()

# if user provided access and secret keys, check if credentials exist inside ~/.aws/credentials. If not,
# set credentials to None
if access_key and secret_key:
    if not check_aws_credentials(access_key, secret_key):
        access_key = None
        secret_key = None


# If either Access or Secret Key doesn't exist, prompt the user for input
if not access_key or not secret_key:
    access_key, secret_key = prompt_aws_credentials()

# Continually prompt for credentials until a valid combination is entered.
    while not check_aws_credentials(access_key, secret_key):
        messagebox.showerror("Error", "Invalid credentials. Please try again.")
        access_key, secret_key = prompt_aws_credentials()

# Creates a new credential file at ~/.aws/credentials
    create_aws_credentials_file(access_key, secret_key)

# Iterator used to print total files uploaded at the end of the script
upload_count = 0


messagebox.showinfo("Information", "Please select the local files to upload.")
# Main while loop to allow infinite file uploads
while True:
    # Opens finder window to allow user to search local machine for photos to upload, supports multi-selection
    file_paths = browse_files()

    # Grabs the file path of each selected photo
    for file_path in file_paths:
        wasabiFileLocation = os.path.basename(file_path)

        try:
            bucket_name = 'OBFUSCATED FOR DEMO PURPOSES'

            # Establishing the s3 Resource using Boto3
            s3 = boto3.resource(
                's3',
                endpoint_url='OBFUSCATED FOR DEMO PURPOSES',
                aws_access_key_id=access_key,
                aws_secret_access_key=secret_key
            )
            # Actual upload command
            s3.meta.client.upload_file(file_path, bucket_name, f"productimages/{wasabiFileLocation}")
            upload_count += 1

            # Generic error handling if the file can't be uploaded
        except Exception as e:
            error_message = f"Error uploading file '{wasabiFileLocation}':\n{str(e)}"
            messagebox.showerror("Error", error_message)

    choice = messagebox.askyesno("Confirmation", "Do you want to upload more files?")
    
    if not choice:
        break

summary_message = f"{upload_count} item(s) successfully uploaded!"
messagebox.showinfo("Summary", summary_message)

root.destroy()
sys.exit()
